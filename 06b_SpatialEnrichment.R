...
# - table of distances from mounds to nearest settlement generated in ArcGIS 
# 
# Process
# 1.Load data
...
# Load shapefiles of mounds, urban areas, boundaries etc. generated by ArcGIS (1 spatial duplicate)
...
crs(bg_border)
st_crs(Y_region)
all.equal(st_crs(Yam),st_crs(mounds)) # LOOKS GREAT
all.equal(st_crs(bg_border),st_crs(mounds)) # LOOKS GREAT
all.equal(st_crs(Y_towns),st_crs(mounds)) # LOOKS GREAT
all.equal(st_crs(towns),st_crs(mounds))
...
enviro_Yammnds$distBG <- st_distance(mounds,bg_border)
enviro_Yammnds[1:3,]
...
# enviro_Yammnds$distTown <- st_distance(mounds,towns) # creates a list which is unusable at moment
...
nearest <-  try(st_nearest_feature(mounds, towns))
distSettle <- st_distance(mounds, towns[nearest,], by_element = TRUE)
...
dim(enviro_Yammnds)
enviro_Yammnds$distTown <- st_distance(mounds, towns[nearest,], by_element = TRUE)

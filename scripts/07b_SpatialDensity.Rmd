...
mnd_digit$MapSymbol[grep("Hairy*",x = mnd_digit$MapSymbol)]
# Intersect
# Visual review
plot(Y_region$geometry,  col = "pink")
plot(st_intersection(mnd_digit$geometry, Y_region), add = TRUE)
Y_digit <- st_intersection(mnd_digit,Y_region)
...
Y_digmnd <- Y_digit %>% filter(grepl("Hairy*", MapSymbol))
...
```{r}
plot(Y_region, col='pink')
plot(Y_digmnd, cex= 0.5, add=TRUE)

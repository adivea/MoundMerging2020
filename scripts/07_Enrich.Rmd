...
```{r yambol-mnds}
Yam_mnds <- st_intersection(mnd_sp, Y_region$geometry)
...
```{r export-features}
# RDS
write_rds(master_sp, "../output_data/enriched_dd_features.rds")
master_sp %>%
  # group_by(Type) %>% 
  # tally()
  filter(grepl("Mound|Other|Uncertain", Type))%>% 
...
write_rds(mnd_sp, "../output_data/enriched_mounds_Yam.rds") # Yambol mounds

# shapefiles
master_sp %>% 
  dplyr::select(-TopoID) %>% 
  st_write("../output_data/enriched_features2023.shp", append = F)

mnd_sp %>% 
  st_write("../output_data/enriched_mnds_Yam.geojson", append = F)  # Yambol mounds
...
master_sp %>%
  ...
  dplyr::select(-TopoID) %>% 
  filter(grepl("Mound|Other|Uncertain", Type))%>% 
  st_write("../output_data/enriched_mounds2023.shp", append = F)
